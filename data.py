...
def save_model_results(model, data_dict):
    ...
    data_dict['sigma_local'] = np.append(data_dict['sigma_local'], np.mean(model.sigma_local))
    data_dict['sigma_global'] = np.append(data_dict['sigma_global'], np.mean(model.sigma_global))
